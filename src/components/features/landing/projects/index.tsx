...
import { GetProjects } from "@/src/api/services/projects";

type Project = {
  title: string; category: string; tech: string[];
  desc: string; github: string; live: string; year: string; status: string;
};

function normalizeProject(raw: any): Project {
  const STATUS_MAP: Record<string, string> = { "live": "Live", "in progress": "In Progress", "archived": "Archived" };
  return {
    title: raw.title ?? "",
    category: raw.category ?? "Full-Stack",
    tech: Array.isArray(raw.tech) ? raw.tech : (raw.tech_stack ?? "").toString().split(",").map((t: string) => t.trim()).filter(Boolean),
    desc: raw.description ?? raw.desc ?? "",
    github: raw.github_url ?? raw.github ?? "",
    live: raw.live_url ?? raw.live ?? "",
    year: raw.year?.toString() ?? "",
    status: STATUS_MAP[raw.status] ?? raw.status ?? "Live",
  };
}
  ...
  useEffect(() => {
    GetProjects()
      .then((data) => {
        ...
        setAllProjects(list.map(normalizeProject));
      })
      .catch(console.error)
      ...
      <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 pt-24 pb-20 relative overflow-hidden transition-colors duration-300">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading && <p className="col-span-3 text-center text-gray-400 py-12">Loading projects...</p>}
            {!loading && filtered.length === 0 && <p className="col-span-3 text-center text-gray-400 py-12">No projects found.</p>}
            {filtered.map((p, i) => (
