...
import { GetServices } from "@/src/api/services/services";
import { gradientClasses } from "@/src/components/features/admin/services";
  ...
  useEffect(() => {
    GetServices()
      .then((data) => {
        ...
        setServices(list.filter((s: ServiceItem) => s.visible));
      })
      .catch(console.error);
  }, []);
    ...
    <section id="services" className="py-24 bg-white dark:bg-transparent">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((s, i) => {
            const features = Array.isArray(s.features) ? s.features : s.features.split(",").map((f) => f.trim()).filter(Boolean);
            const gradCls = gradientClasses[s.color_gradient] ?? "from-violet-600 to-purple-700";
            return (
              ...
              <div className="flip-card h-72 w-full cursor-pointer">
                <div className="flip-card-inner">
                  <div className="flip-card-front relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 flex flex-col overflow-hidden">
                    <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${gradCls}`} />
                    <span className="absolute top-4 right-4 text-xs font-bold text-gray-700">0{i + 1}</span>
                    <div className={`w-14 h-14 rounded-xl mb-5 shadow-lg float-anim bg-gradient-to-br ${gradCls}`} style={{ animationDelay: `${i * 0.4}s` }} />
                    ...
                    <p className="mt-auto text-xs font-medium text-violet-400">Hover to see more →</p>
                  </div>
                  <div className={`flip-card-back relative rounded-2xl p-6 flex flex-col justify-between overflow-hidden bg-gradient-to-br ${gradCls}`}>
                    ...
                    <span className="text-white/70 text-xs mt-4">Available for hire</span>
                  </div>
                </div>
              </div>
            </Reveal>
