...
function normalize(raw: any, i: number): Testimonial {
  ...
  return {
    id: raw.id,
    name,
    role: raw.client_role ?? raw.role ?? raw.position ?? "",
    avatar: name.split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase(),
    rating: raw.rating ?? 5,
    text: raw.review ?? raw.text ?? raw.message ?? "",
...
function TestimonialsSlider({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  if (total === 0) return <p className="text-center text-gray-400">No testimonials yet.</p>;
