...
import { SendQueries } from "@/src/api/services/queries";
...
export default function ContactSection() {
  ...
  async function onSubmit(data: FormValues) {
    ...
    setSubmitError(null);
    try {
      await SendQueries(data.name, data.email, data.subject, data.message);
      setSent(true);
      reset();
    } catch {
              ...
              {sent ? (
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
                  ...
                  <button type="submit" disabled={!isValid || !isDirty || submitting}
                    ...
                    {!submitting && <svg className="w-4 h-4 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
