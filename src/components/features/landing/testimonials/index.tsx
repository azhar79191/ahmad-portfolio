...
import { GetTestimonials } from "@/src/api/services/testimonials";
...
function normalize(raw: any, i: number): Testimonial {
  const name = raw.client_name ?? raw.name ?? "Anonymous";
  return {
    name,
  ...
  useEffect(() => {
    GetTestimonials()
      .then((data) => {
        ...
        setTestimonials(list.map(normalize));
      })
      .catch(console.error)
      ...
      <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 pt-24 pb-20 relative overflow-hidden transition-colors duration-300">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading && <p className="col-span-3 text-center text-gray-400 py-12">Loading testimonials...</p>}
            {!loading && testimonials.length === 0 && <p className="col-span-3 text-center text-gray-400 py-12">No testimonials yet.</p>}
